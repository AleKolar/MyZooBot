...
В общем одумался подключил SQlite3 - понятно не маштабируема, но для учебных целей оч.хорошо(все как Postgresql, только без возможности подсоединения). (Вопросы теста были в БД, но было не удобно мониторить и я их опять занес - Да не solid,
зато информативно!)
Данный aiogram Бот прямолинеен. Как только нашел, способ исполнить задуманный функционал отбросил все лишнее для надежности. Так как маштабируемости надо добиваться залил его на Repl.it https://replit.com/@alekolar17982/Python 
...
По задумке TheQuizzooBot должен работать автономно, так как захостили на UptimeRobot и flask-сообщение: I'm Alive внушает оптимизм, но без вложений его конечно так не задеплоишь, и я забыл прописать кусочек кода, чтоб считал ping. 
Мне очень жаль(Заказчику, конечно так не скажешь), но это только потому, что я работу работаю времязабирающую. И надо как-то запустить Docker! Запускался бы Docker
и жизнь была бы другой. Спасибо, что дочитали!
